...
const DEFAULT_MODELS: Record<AIProvider, ModelInfo[]> = {
    ...
    anthropic: [{ id: "claude-haiku-4-5-20251001", name: "Claude Haiku 4.5" }],
    groq: [{ id: "llama-4-maverick", name: "Llama 4 Maverick 400B" }],
    deepseek: [{ id: "deepseek-chat", name: "DeepSeek Chat" }],
    gemini: [{ id: "models/gemini-2.5-flash", name: "Gemini 2.5 Flash" }],
    ollama: [{ id: "gemma3:4b", name: "Gemma3 - 4b" }],
};
...
class ModelService {
    ...
    private async fetchOpenAIModels(apiKey: string): Promise<ModelInfo[]> {
        try {
            ...
            if (!response.ok) return DEFAULT_MODELS.openai;
    ...
    private async fetchAnthropicModels(apiKey: string): Promise<ModelInfo[]> {
        try {
            ...
            if (!response.ok) return DEFAULT_MODELS.anthropic;
    ...
    private async fetchGroqModels(apiKey: string): Promise<ModelInfo[]> {
        try {
            ...
            if (!response.ok) return DEFAULT_MODELS.groq;
    ...
    private async fetchDeepSeekModels(apiKey: string): Promise<ModelInfo[]> {
        try {
            ...
            if (!response.ok) return DEFAULT_MODELS.deepseek;
    ...
    private async fetchGeminiModels(apiKey: string): Promise<ModelInfo[]> {
        try {
            ...
            if (!response.ok) return DEFAULT_MODELS.gemini;
    ...
    private async fetchOllamaModels(): Promise<ModelInfo[]> {
        try {
            ...
            if (!response.ok) return DEFAULT_MODELS.ollama;
...
let singleton: ModelService | null = null;

export function getModelService(): ModelService {
    if (!singleton) {
        singleton = new ModelService();
    }
    return singleton;
}
