...
export async function getStoredKeys(): Promise<KeyStore> {
    return readFromStore<KeyStore>(STORAGE_FILES.API_KEYS, KEYSTORE_FALLBACK);
}

export async function storeKey(provider: AIProvider, apiKey: string): Promise<void> {
...
export async function getKey(provider: AIProvider): Promise<string | undefined> {
